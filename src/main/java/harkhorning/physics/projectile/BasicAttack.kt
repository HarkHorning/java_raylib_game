...
class BasicAttack(val targetP: Vector2) {
    ...
    fun getCollisionRect() : Rectangle
    {
        return Rectangle().x(p.x() - radius.x()).y(p.y() - radius.y()).width(radius.x() * 2).height(radius.y() * 2)
    }

    fun update(lockScroll: Vector2)
    {
    ...
    fun draw()
    {
        val origin = Vector2().x(radius.x()).y(radius.y())
        DrawTexturePro(sprite, drawRect, getCollisionRect(),
            origin, angleDeg, WHITE)
    }
}